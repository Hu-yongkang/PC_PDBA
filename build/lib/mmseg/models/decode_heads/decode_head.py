...
class PixelContrastLoss(nn.Module, ABC):
    ...
    def _hard_anchor_sampling(self, X, y_hat, y):
        ...
        for ii in range(batch_size):
            ...
            this_classes = [x for x in this_classes if (x != self.ignore_label and x != 255  )]
...
class BaseDecodeHead(BaseModule, metaclass=ABCMeta):
    ...
    @auto_fp16()
    @abstractmethod
    def forward(self, inputs):  # 作用是转为fp16精度的
        """Placeholder of forward function.转发函数的占位符  """
        pass

    def forward_train(self,     # feature_outputs={'seg': out(4个concat后分类), 'embed': emb(4个concat后mlp)}
                      inputs,   # x=inputs=[f1,f2,f3,f4]
                      feature_outputs,  # 新增
                      stage,            # 新增
                      img_metas,
                      gt_semantic_seg,
        ...
        if p_label:
            losses = dict()
            losses['loss_seg']=0
            losses['acc_seg'] = torch.FloatTensor(0)
            target = torch.squeeze(gt_semantic_seg, dim=1)
            h, w = target.size(1), target.size(2)
            seg = feature_outputs['seg'] # feature_outputs={'seg': out(4个concat后分类), 'embed': emb(4个concat后mlp)}
            embedding = feature_outputs['embed']
            _, predict = torch.max(seg, 1)
            loss_contrast = self.contrast_criterion(embedding, target, predict)
            losses['loss_seg'] = losses['loss_seg'] + self.loss_weight * loss_contrast * float(torch.mean(seg_weight))
            return losses                      # 结束
            


        else:
            seg_logits, _ = self.forward(inputs)   # # x=inputs=[f1,f2,f3,f4],得到伪标签？很奇怪
            losses = self.losses(seg_logits, gt_semantic_seg, seg_weight) # 分割损失loss['loss_seg']，精度损失loss['acc_seg']
            if stage == 1:                          # 新增-------------------------------------
                # Pixel Contrast
                target = torch.squeeze(gt_semantic_seg, dim=1)
                h, w = target.size(1), target.size(2)
                ...
                _, predict = torch.max(seg, 1)
                loss_contrast = self.contrast_criterion(embedding, target, predict)
                losses['loss_seg'] = losses['loss_seg'] + self.loss_weight * loss_contrast * 0.1
                return losses                      # 结束
            else:
                return losses
