...
class PairUrbanDataset(BaseDataSet):
    def __init__(self, **kwargs):
        ...
        super(PairUrbanDataset, self).__init__(**kwargs)
    ...
    def _set_files(self):   #  此处指向路径‘dataloaders/city_splits2/的txt文件’
        prefix = "dataloaders/Urban_splits{}".format(self.datalist)
...
class PairUrban(BaseDataLoader):
    def __init__(self, kwargs):
        
        self.MEAN = [0.292692,0.310419,0.304710]     # ----------------------------------------------
        self.STD = [0.161328,0.149184,0.144937]
        ...
        self.dataset = PairUrbanDataset(**kwargs)
        shuffle = False
        #dist_sampler = torch.utils.data.distributed.DistributedSampler(self.dataset, shuffle=sampler_shuffle)

        super(PairUrban, self).__init__(self.dataset, self.batch_size, shuffle, num_workers, val_split=None, dist_sampler=None)


class UrbanDataset(BaseDataSet):
    def __init__(self, **kwargs):
        self.num_classes = 8

        self.datalist = kwargs.pop("datalist")
        self.palette = pallete.get_voc_pallete(self.num_classes)
        super(UrbanDataset, self).__init__(**kwargs)
    ...
    def _load_data(self, index):
        image_path = os.path.join(self.root, self.files[index][:])
        image = np.asarray(Image.open(image_path), dtype=np.float32)  # 仅合适Urban的测试集
...
class Urban(BaseDataLoader):
    def __init__(self, kwargs):
        ...
        self.dataset = UrbanDataset(**kwargs)
        shuffle = False
        #dist_sampler = torch.utils.data.distributed.DistributedSampler(self.dataset, shuffle=sampler_shuffle)

        super(Urban, self).__init__(self.dataset, self.batch_size, shuffle, num_workers, val_split=None, dist_sampler=None)
